---
title: Physics
type: concept
status: active
created: 2026-04-07
updated: 2026-06-17
summary: 物理学科总览页，用来组织理论分支、模型、实验与和天文学的连接。
tags: [physics, discipline]
sources: []
---

# Physics

## Scope

这页作为物理总览页，负责归纳力学、电磁学、热学、相对论、量子理论等资料，并连接到相关数学与天文学页面。

对应的单学科资料网络入口是 [物理 Knowledge Network](../domains/physics.md)。

## Recommended Subtopics

- 经典力学
- 电磁学
- 热力学与统计物理
- 相对论
- 量子力学
- 天体物理的物理基础

## First Pages To Build

- 牛顿力学核心框架
- 守恒律与对称性
- 引力模型与轨道运动
- 观测与测量误差的物理背景
- 现代天文学常用的物理概念

## Cross-Disciplinary Links

- [Mathematics](./math.md): 物理建模与推导依赖数学语言。
- [Modern Astronomy](./modern-astronomy.md): 天文学中的天体物理、轨道力学、辐射机制都以物理为基础。
- [Tibetan Astronomy Calendar](./tibetan-astronomy-calendar.md): 可对照传统历算中的天体周期模型与现代物理解释。

## Open Questions

- 你的物理资料更偏教材体系、专题研究，还是服务于现代天文学理解？
- 是否需要单独建立“物理中的天文相关概念”专题页？

## Related

- [物理 Knowledge Network](../domains/physics.md)
- [Mathematics](./math.md)
- [Modern Astronomy](./modern-astronomy.md)
- [Tibetan Astronomy Calendar](./tibetan-astronomy-calendar.md)

## Sources

- 暂无，等待首批物理资料 ingest
